...
vowel_mfccs_array = {}
vowel_mfccs_average_array = {}
for i in vowel_files:
	vowel_mfccs_array[i] = []
	vowel_mfccs_average_array[i] = []
...
for file in vowel_files:
	for folder in training_vowel_folders:
		...
		'''
			c. Tính vector đặc trưng cho 1 nguyên âm của 1 người nói = Trung bình cộng của M vector MFCC của M khung
				thuộc vùng ổn định
		'''
		...
		'''
			d. Tính vector đặc trưng cho 1 nguyên âm của nhiều người nói = Trung bình cộng của các vector đặc trưng
				cho 1 nguyên âm của 21 người nói
		'''

		vowel_mfccs_array[file].append(mfccs_average_array)
...
'''
	3.	SO KHỚP VECTOR MFCC CỦA TÍN HIỆU NGUYÊN ÂM ĐẦU VÀO VỚI 5 VECTOR ĐẶC TRUNG ĐÃ TRÍCH XUẤT CỦA 5 NGUYÊN ÂM
		ĐƯA RA KẾT QUẢ NHẬN DẠNG NGUYÊN ÂM: TÍNH 5 KHOẢNG CÁCH EUCLIDEAN GIỮA 2 VECTOR VÀ ĐƯA RA QUYẾT ĐỊNH
...
for file in vowel_files:
	for folder in testing_vowel_folders:
		...
		euclidean = 1000
		presumable_results = ''
		for v in vowel_mfccs_average_array:
			new_euclidean = np.linalg.norm(mfccs_average_array - vowel_mfccs_average_array[v])
